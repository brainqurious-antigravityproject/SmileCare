...
function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Register the user (does NOT auto-login anymore - redirects to login page)
      ...
      // After successful signup -> redirect to login with pre-filled email
      const loginHref = `/login?redirect=${encodeURIComponent(redirectTo)}&email=${encodeURIComponent(form.email)}&registered=1`;
    ...
    <main className="min-h-[calc(100vh-5rem)] flex items-center justify-center bg-background-light px-4 py-16">
      <div className="max-w-md w-full bg-pearl rounded-2xl shadow-xl border border-primary/10 p-10">
        ...
        <div className="flex flex-col gap-3 mt-6 mb-6">
          <a
            href="#"
            onClick={(e) => { e.preventDefault(); alert("Google sign-up coming soon — configure NEXT_PUBLIC_GOOGLE_CLIENT_ID."); }}
          ...
          <a
            href="#"
            onClick={(e) => { e.preventDefault(); alert("Apple sign-up coming soon — configure Apple OAuth credentials."); }}
            className="flex items-center justify-center gap-3 w-full border border-slate-200 rounded-xl py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 active:scale-[0.98] transition-all"
          >
